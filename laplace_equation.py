            ...
            try:
                if sym.simplify(results).is_constant():
                    ...
                    if re.search('tan', string):
                        ...
                        Y = Y.subs(k, n*pi/initial_value)
                        u_y = u_y.subs(k, n*pi/initial_value)

                    else:
                        X = X.subs(solved_boundary[0], solved_boundary[1])
                        u_x = u_x.subs(solved_boundary[0], solved_boundary[1])
                        Y = Y.subs(solved_boundary[0], solved_boundary[1])
                        u_y = u_y.subs(solved_boundary[0], solved_boundary[1])
                else:
                    function = results
...
total_function = b*(Y*X).subs({A: 1, B: 1, C: 1, D: 1})
print(total_function)

if function_type == 'X' or function_type == 'Y':
